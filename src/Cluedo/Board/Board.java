...
public class Board {
    ...
    public int drawMove(String input, int diceNumber, Player p, Graphics g) {
        ...
        g.drawOval((position.getCol() * 20) + 10, (position.getRow() * 20) + 10, 20, 20);
    ...
    public void movePlayerToPlayer(Player first, Player second) {
        ...
        firstPos = findPlayer(first);
        try {
            secondPos = findPlayer(second);
        } catch (IllegalStateException e) {
            return;
        }
        RoomCard.RoomType room = ((RoomTile)getTileAt(firstPos)).getRoom();
        Position newSecondPos;
        // attempt 1
        for(BoardTile.MoveDirection d : BoardTile.MoveDirection.values()) {
            newSecondPos = firstPos.move(d);
            BoardTile tile = getTileAt(newSecondPos);
            if((tile instanceof RoomTile) && ((RoomTile) tile).getRoom() == room && tile.canMoveHere(d)) {
                RoomTile.movePlayer((RoomTile) getTileAt(secondPos), (RoomTile) getTileAt(newSecondPos));
                break;
            }
        }
    }
